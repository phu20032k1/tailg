...
async function getAuthorizedReport(request:NextRequest,id:string){
 ...
 if(!["leader","commander"].includes(session.role))return {response:NextResponse.json({ok:false,error:"Bạn chỉ có quyền xem báo cáo, không có quyền sửa dữ liệu đội."},{status:403})};
...
function buildChangeSummary(report:any, body:z.infer<typeof updateSchema>, workers:number, technicalStaff:number, beforeTasks:number){
 const changes:string[]=[];
 if(report.report_date!==body.reportDate)changes.push(`ngày ${report.report_date} → ${body.reportDate}`);
 if(Number(report.workers||0)!==workers)changes.push(`công nhân ${Number(report.workers||0)} → ${workers}`);
 if(Number(report.technical_staff||0)!==technicalStaff)changes.push(`kỹ thuật ${Number(report.technical_staff||0)} → ${technicalStaff}`);
 if(beforeTasks!==body.tasks.length)changes.push(`công việc ${beforeTasks} → ${body.tasks.length}`);
 if((report.weather_morning||"")!==body.weatherMorning||(report.weather_noon||"")!==body.weatherNoon||(report.weather_afternoon||"")!==body.weatherAfternoon||(report.weather_evening||"")!==body.weatherEvening)changes.push("thời tiết");
 if((report.issue_text||"")!==body.issueText)changes.push("vướng mắc/ghi chú");
 if((report.raw_message||"")!==body.rawMessage)changes.push("nội dung báo cáo gốc");
 return changes.length?`Đã sửa: ${changes.join("; ")}`:"Đã lưu lại nội dung báo cáo";
}
...
export async function PATCH(request:NextRequest,context:{params:Promise<{id:string}>}){
 ...
 try{
  ...
  if(body.reportDate!==report.report_date){const {data:duplicate}=await db.from("daily_reports").select("id").eq("leader_id",report.leader_id).eq("report_date",body.reportDate).neq("id",id).maybeSingle(); if(duplicate)return NextResponse.json({ok:false,error:"Đội này đã có báo cáo ở ngày đã chọn. Hãy sửa báo cáo của ngày đó thay vì tạo trùng.",existingReportId:duplicate.id},{status:409});}
  ...
  if(laborBefore.error||equipmentBefore.error||tasksBefore.error)throw laborBefore.error||equipmentBefore.error||tasksBefore.error;
  ...
  const changeSummary=buildChangeSummary(report,body,workers,technicalStaff,(tasksBefore.data||[]).length);
  ...
  const {error:auditError}=await db.from("report_edit_history").insert({report_id:id,edited_by:session.id,edited_by_name:session.fullName||session.username||"Người dùng",edited_at:now,before_data:beforeData,after_data:afterData,change_summary:changeSummary}); if(auditError)throw auditError;
  return NextResponse.json({ok:true,workers,technicalStaff,editedAt:now,changeSummary});
