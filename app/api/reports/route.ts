...
const reportSchema = z.object({
  reportDate:z.string().regex(/^\d{4}-\d{2}-\d{2}$/), rawMessage:z.string().max(20000).optional().default(""), issueText:z.string().max(2000).optional().default(""),
  weatherMorning:z.string().trim().max(120).optional().default(""), weatherNoon:z.string().trim().max(120).optional().default(""), weatherAfternoon:z.string().trim().max(120).optional().default(""), weatherEvening:z.string().trim().max(120).optional().default(""),
...
export async function POST(request:NextRequest){
  ...
  try{
    ...
    if(body.foundationUpdates.length){
      ...
      selectedFoundations=data||[]; if(selectedFoundations.length!==ids.length||selectedFoundations.some(f=>f.owner_id!==leaderId))return NextResponse.json({ok:false,error:"Có móng không thuộc đội đang báo cáo. Vui lòng chọn lại."},{status:400});
    }
    const {data,error}=await db.rpc("save_daily_report_v3",{p_report_date:body.reportDate,p_leader_id:leaderId,p_raw_message:body.rawMessage||null,p_issue_text:body.issueText||null,p_labor:body.labor.map(i=>({category_code:i.categoryCode,label:i.label,crew_name:i.crewName||null,headcount:i.headcount,counts_as_worker:i.countsAsWorker,sort_order:i.sortOrder})),p_equipment:body.equipment.map(i=>({equipment_name:i.equipmentName,quantity:i.quantity,unit:i.unit,note:i.note||null,sort_order:i.sortOrder})),p_tasks:body.tasks.map(i=>({kind:i.kind,area_label:i.areaLabel||null,description_vi:i.descriptionVi,description_zh:i.descriptionZh||null,sort_order:i.sortOrder}))});
