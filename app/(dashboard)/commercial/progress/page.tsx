...
export default async function ProjectProgressPage(){
  ...
  if(!["commander","leader","khkt","director","finance"].includes(user.role)) return null;
  const data=await getCommercialData();
  const packages=(user.role==="leader"?data.packages.filter((item:any)=>item.owner_type==="team"&&item.owner_name===user.fullName):data.packages) as any[];
  ...
  return <div className="commercial-page"><section className="commercial-page-head"><div><span className="eyebrow">TIẾN ĐỘ TỔNG THỂ</span><h1>Tiến độ theo đầu mục, đội và nhà thầu</h1><p>Mỗi đầu mục có khối lượng giao, khối lượng ngày, lũy kế, ngày cam kết và trọng số để tổng hợp tiến độ toàn dự án.</p></div><ChartNoAxesCombined size={30}/></section>
