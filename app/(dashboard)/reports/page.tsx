import { CalendarDays, Camera, CloudSun, Cog, HardHat, History, ListChecks, Search } from "lucide-react";
...
function editTime(value?: string | null) {
  if (!value) return "";
  return new Intl.DateTimeFormat("vi-VN", { hour:"2-digit", minute:"2-digit", day:"2-digit", month:"2-digit", year:"numeric", timeZone:"Asia/Ho_Chi_Minh" }).format(new Date(value));
}
...
export default async function ReportsPage({ searchParams }: { searchParams: Promise<{ from?: string; to?: string }> }) {
  ...
  const canEdit = user.role === "leader" || user.role === "commander";

  return <>
    <section className="page-title-row"><div><span className="eyebrow">NHẬT KÝ THI CÔNG</span><h1>Lịch sử báo cáo ngày</h1><p>{user.role === "commander" ? "Xem, lọc theo ngày, chỉnh sửa hoặc xóa báo cáo của 6 đội. Mỗi lần sửa đều có lịch sử." : "Xem báo cáo theo ngày; đội trưởng có thể sửa lại dữ liệu ngày cũ."}</p></div><div className="page-title-icon"><History size={25} /></div></section>

    <section className="panel history-filter-panel"><div className="panel-body"><form className="history-calendar-filter" method="get"><div className="calendar-filter-title"><CalendarDays size={19} /><strong>Lọc theo thời gian</strong></div><label><span>Từ ngày</span><input type="date" name="from" defaultValue={query.from || ""} /></label><label><span>Đến ngày</span><input type="date" name="to" defaultValue={query.to || ""} /></label><button className="button secondary" type="submit"><Search size={17} /> Xem báo cáo</button>{hasRange ? <a className="button ghost" href="/reports">Bỏ lọc</a> : null}</form></div></section>

    <div className="history-stack section-gap">
      {reports.map((report) => <article className="panel report-card lazy-section" key={report.id}>
        <div className="report-card-head"><div><span className="eyebrow">{formatDate(report.report_date)}</span><h2>{report.leader?.full_name || "Đội thi công"}</h2><small>Cập nhật: {editTime(report.updated_at)}</small></div><div className="report-card-tools"><div className="headcount-chip"><HardHat size={17} /><strong>{report.workers}</strong> công nhân <span>+ {report.technical_staff} kỹ thuật</span></div>{canEdit ? <ReportActions reportId={report.id} /> : null}</div></div>

        <div className="management-weather-team management-weather-four report-weather-row"><div><span>Sáng</span><strong>{report.weather_morning || "-"}</strong></div><div><span>Trưa</span><strong>{report.weather_noon || "-"}</strong></div><div><span>Chiều</span><strong>{report.weather_afternoon || "-"}</strong></div><div><span>Tối</span><strong>{report.weather_evening || "-"}</strong></div></div>
