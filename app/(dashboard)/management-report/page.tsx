    ...
    <div className="management-report-page">
      <section className="management-report-heading">
        <div>
          ...
          <p>Dữ liệu tự cập nhật từ báo cáo của 6 đội trưởng trong ngày {formatDate(date)}.</p>
        </div>
      ...
      <section className="management-overview-grid">
        <article className="panel management-overview-card">
          ...
          <div className="panel-body management-weather-row"><div><span>Buổi sáng</span><strong>{data.summary.weatherMorning}</strong></div><div><span>Buổi chiều</span><strong>{data.summary.weatherAfternoon}</strong></div></div>
        </article>
        <article className="panel management-overview-card">
          <div className="panel-head"><div><span className="eyebrow">NHÂN LỰC</span><h2>Cơ cấu toàn dự án</h2></div><Users size={20}/></div>
          <div className="panel-body management-total-list">
            {data.summary.laborTotals.length ? data.summary.laborTotals.map((item) => <div key={item.label}><span>{item.label}</span><strong>{item.headcount}</strong></div>) : <div className="empty-state">Chưa có dữ liệu nhân lực.</div>}
          </div>
        </article>
        <article className="panel management-overview-card">
          <div className="panel-head"><div><span className="eyebrow">THIẾT BỊ</span><h2>Tổng hợp máy móc</h2></div><Wrench size={20}/></div>
          <div className="panel-body management-total-list">
            {data.summary.equipmentTotals.length ? data.summary.equipmentTotals.map((item) => <div key={`${item.name}-${item.unit}`}><span>{item.name}</span><strong>{item.quantity} {item.unit}</strong></div>) : <div className="empty-state">Chưa có dữ liệu thiết bị.</div>}
          </div>
        </article>
      </section>
      ...
      <section className="management-team-index panel">
        ...
        <div className="panel-body management-team-table-wrap">
          <table className="management-team-table">
            <thead><tr><th>STT</th><th>Đội trưởng</th><th>Phạm vi</th><th>Trạng thái</th><th>Công nhân</th><th>Kỹ thuật</th><th>Công việc</th><th>Ảnh</th></tr></thead>
            <tbody>{data.teamRows.map((team) => <tr key={team.leader.id}><td>{team.stt}</td><td><strong>{team.leader.full_name}</strong></td><td>{team.zoneLabel || "Chưa gán khu vực"}</td><td><span className={`management-status ${team.reported ? "done" : "missing"}`}>{team.reported ? "Đã báo cáo" : "Chưa báo cáo"}</span></td><td>{team.workers}</td><td>{team.technical}</td><td>{team.tasks.length}</td><td>{team.photos.length}</td></tr>)}</tbody>
          </table>
        </div>
      </section>
      ...
      <section className="management-team-report-list">
        {data.teamRows.map((team) => (
          <article className={`management-team-report ${team.reported ? "" : "is-missing"}`} key={team.leader.id}>
            <header className="management-team-report-head">
              <div className="management-team-number">{String(team.stt).padStart(2, "0")}</div>
              <div className="management-team-title"><span>ĐỘI TRƯỞNG</span><h2>{team.leader.full_name}</h2><p>{team.zoneLabel || "Chưa gán khu vực"}</p></div>
              <div className={`management-report-state ${team.reported ? "done" : "missing"}`}><strong>{team.reported ? "ĐÃ BÁO CÁO" : "CHƯA BÁO CÁO"}</strong>{team.report ? <span>{submittedTime(team.report.submitted_at || team.report.updated_at)}</span> : null}</div>
            </header>

            {!team.reported ? <div className="management-missing-body">Chưa nhận được báo cáo của đội trong ngày {formatDate(date)}.</div> : <>
              <div className="management-team-kpis">
                <div><span>Công nhân</span><strong>{team.workers}</strong></div>
                <div><span>Kỹ thuật</span><strong>{team.technical}</strong></div>
                <div><span>Tổng nhân lực</span><strong>{team.totalPeople}</strong></div>
                <div><span>Móng phụ trách</span><strong>{team.foundationCount}</strong></div>
                <div><span>Móng hoàn thành</span><strong>{team.completedFoundations}</strong></div>
                <div><span>Tiến độ móng</span><strong>{formatPercent(team.foundationProgress)}%</strong></div>
              </div>

              <div className="management-team-grid">
                <section>
                  <h3>Nhân lực chi tiết</h3>
                  {team.labor.length ? <table className="management-detail-table"><thead><tr><th>Nhóm</th><th>Tổ / phụ trách</th><th>SL</th></tr></thead><tbody>{team.labor.map((item) => <tr key={item.id}><td>{item.label}</td><td>{item.crew_name || "-"}</td><td>{item.headcount}</td></tr>)}</tbody></table> : <p className="management-empty">Chưa có chi tiết nhân lực.</p>}
                </section>
                <section>
                  <h3>Máy móc thiết bị</h3>
                  {team.equipment.length ? <table className="management-detail-table"><thead><tr><th>Thiết bị</th><th>SL</th><th>Đơn vị</th></tr></thead><tbody>{team.equipment.map((item) => <tr key={item.id}><td>{item.equipment_name}</td><td>{item.quantity}</td><td>{item.unit}</td></tr>)}</tbody></table> : <p className="management-empty">Chưa có thiết bị được ghi nhận.</p>}
                </section>
              </div>

              <section className="management-work-section">
                <h3>Công việc trong ngày</h3>
                {team.tasks.length ? <div className="management-task-list">{team.tasks.map((task, index) => <div key={task.id}><span>{String(index + 1).padStart(2, "0")}</span><div><strong>{task.area_label || "Khu vực thi công"}</strong><p>{task.description_vi}</p>{task.description_zh ? <small>{task.description_zh}</small> : null}</div></div>)}</div> : <p className="management-empty">Chưa có công việc chi tiết.</p>}
              </section>
              ...
              <div className="management-weather-team"><div><span>Thời tiết sáng</span><strong>{team.weatherMorning || "Chưa ghi nhận"}</strong></div><div><span>Thời tiết chiều</span><strong>{team.weatherAfternoon || "Chưa ghi nhận"}</strong></div></div>
      ...
      <footer className="management-print-footer print-only">
        <div><span>Người tổng hợp</span><strong>Ban điều hành dự án</strong></div>
        <div><span>Chỉ huy trưởng</span><strong>Phan Viết Tùng</strong></div>
      </footer>
    </div>
  );
}
