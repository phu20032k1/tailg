...
export function DailyReportForm({ user, leaders, foundations }: { user:SessionUser; leaders:Leader[]; foundations:FoundationOption[] }) {
  ...
  const [weatherNoon,setWeatherNoon]=useState("Nắng");
  const [weatherAfternoon,setWeatherAfternoon]=useState("Nắng");
  const [weatherEvening,setWeatherEvening]=useState("Nắng");
  ...
  useEffect(()=>{ if(!notice)return; const timer=window.setTimeout(()=>setNotice(null),notice.type==="ok"?2800:5500); return()=>window.clearTimeout(timer); },[notice]);
  ...
  async function translateTask(index:number,text:string){
    ...
    if(source.length<2){setTasks(items=>items.map((item,i)=>i===index?{...item,descriptionZh:"",translationState:"idle"}:item));return;}
    ...
    }catch{setTasks(items=>items.map((item,i)=>i===index&&item.descriptionVi.trim()===source?{...item,translationState:"error"}:item));}
  }
  function scheduleTranslation(index:number,text:string){
    const existing=translationTimers.current[index]; if(existing)window.clearTimeout(existing);
    if(text.trim().length<2){setTasks(items=>items.map((item,i)=>i===index?{...item,descriptionZh:"",translationState:"idle"}:item));return;}
    setTasks(items=>items.map((item,i)=>i===index?{...item,translationState:"loading"}:item));
  ...
  function parseMessage(){
    ...
    if(parsed.teamName&&user.role==="commander"){
      ...
      const matched=leaders.find(l=>normalized.includes(l.full_name.toLocaleLowerCase("vi"))); if(matched)setLeaderId(matched.id);
    }
    ...
    if(parsed.tasks.length){const next=parsed.tasks.map(i=>({...i,areaLabel:i.areaLabel||"",descriptionZh:"",translationState:"loading" as const}));setTasks(next);next.forEach((item,index)=>scheduleTranslation(index,item.descriptionVi));}
    setNotice({type:"ok",title:"Đã bóc tách báo cáo",text:"Kiểm tra lại ngày, thời tiết và số liệu trước khi gửi."});
  }
  ...
  function addFoundation(){const used=new Set(foundationUpdates.map(i=>i.foundationId));const next=availableFoundations.find(f=>!used.has(f.id));if(!next){setNotice({type:"error",title:"Không còn móng để chọn",text:"Danh sách móng của đội đã được chọn hết."});return;}setFoundationUpdates(items=>[...items,{foundationId:next.id,stage:next.current_stage||"Thi công móng",progress:Number(next.progress||0)}]);}
  function addFiles(selected:FileList|File[]|null){const incoming=Array.from(selected||[]);const valid=incoming.filter(isImageFile);const merged=[...files];const seen=new Set(merged.map(fileKey));for(const file of valid){const key=fileKey(file);if(!seen.has(key)){merged.push(file);seen.add(key);}}const kept=merged.slice(0,10);setFiles(kept);setFileInputKey(value=>value+1);if(merged.length>10)setNotice({type:"error",title:"Tối đa 10 ảnh",text:`Đã giữ ${kept.length} ảnh. Bạn có thể xóa ảnh cũ rồi chọn thêm ảnh khác.`});else if(valid.length!==incoming.length)setNotice({type:"error",title:"Có tệp không phải ảnh",text:"Hệ thống chỉ nhận JPG, PNG, WEBP, HEIC và HEIF."});}
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      const response=await fetch("/api/reports",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
        reportDate,rawMessage,weatherMorning,weatherNoon,weatherAfternoon,weatherEvening,
      ...
      const result=await response.json();
      if(response.status===409&&result.existingReportId){setNotice({type:"error",title:"Ngày này đã có báo cáo",text:"Đang mở báo cáo cũ để bạn sửa. Lần sửa sẽ được lưu lịch sử."});window.setTimeout(()=>router.push(`/reports/${result.existingReportId}/edit`),700);return;}
      if(!response.ok)throw new Error(result.error||"Không gửi được báo cáo.");
      ...
      clearSubmittedData();
      setNotice({type:"ok",title:"Đã gửi báo cáo",text:`${submittedWorkers} công nhân · ${submittedTechnical} kỹ thuật${submittedFoundations?` · ${submittedFoundations} móng`:""}${submittedPhotos?` · ${submittedPhotos} ảnh`:""}`});router.refresh();
  ...
  return <>
    <form className="report-form daily-v3-form" onSubmit={submit}>
      <section className="form-section">
        <div className="section-heading"><span>01</span><div><h3>Thông tin báo cáo</h3><p>Có thể chọn ngày trước đây để bổ sung báo cáo còn thiếu.</p></div></div>
        ...
        <div className="weather-entry-grid">
          <div className="weather-entry-title"><CloudSun size={19}/><div><strong>Thời tiết trong ngày</strong><span>Ghi nhận đủ sáng · trưa · chiều · tối để tổng hợp ngày và tuần.</span></div></div>
          ...
          <label className="field"><span>Buổi trưa</span><select value={weatherNoon} onChange={e=>setWeatherNoon(e.target.value)}>{WEATHER_OPTIONS.map(item=><option key={item}>{item}</option>)}</select></label>
          ...
          <label className="field"><span>Buổi tối</span><select value={weatherEvening} onChange={e=>setWeatherEvening(e.target.value)}>{WEATHER_OPTIONS.map(item=><option key={item}>{item}</option>)}</select></label>
        </div>
      ...
      <section className="form-section"><div className="section-heading"><span>03</span><div><h3>Máy móc</h3></div></div><div className="editable-table equipment-table"><div className="editable-row editable-head"><span>Thiết bị</span><span>Số lượng</span><span>Đơn vị</span><span/></div>{equipment.map((item,index)=><div className="editable-row" key={`${item.equipmentName}-${index}`}><input value={item.equipmentName} onChange={e=>changeEquipment(index,{equipmentName:e.target.value})}/><input type="number" min="0" value={item.quantity} onChange={e=>changeEquipment(index,{quantity:Number(e.target.value)})}/><input value={item.unit} onChange={e=>changeEquipment(index,{unit:e.target.value})}/><button type="button" className="icon-button" onClick={()=>setEquipment(items=>items.filter((_,i)=>i!==index))}><Trash2 size={15}/></button></div>)}</div><button className="button ghost" type="button" onClick={()=>setEquipment(items=>[...items,{equipmentName:"",quantity:0,unit:"máy"}])}><Plus size={16}/> Thêm thiết bị</button></section>

      <section className="form-section foundation-report-section"><div className="section-heading"><span>04</span><div><h3>Cập nhật móng</h3></div></div>{availableFoundations.length?<><div className="foundation-update-list">{foundationUpdates.map((item,index)=>{const f=availableFoundations.find(x=>x.id===item.foundationId);return <div className="foundation-update-row" key={`${item.foundationId}-${index}`}><select value={item.foundationId} onChange={e=>{const next=availableFoundations.find(x=>x.id===e.target.value);changeFoundation(index,{foundationId:e.target.value,stage:next?.current_stage||item.stage,progress:Number(next?.progress??item.progress)});}}>{availableFoundations.filter(x=>x.id===item.foundationId||!foundationUpdates.some((u,j)=>j!==index&&u.foundationId===x.id)).map(x=><option key={x.id} value={x.id}>{x.code}</option>)}</select><input value={item.stage} onChange={e=>changeFoundation(index,{stage:e.target.value})} placeholder="Công việc: đổ bê tông, cốt thép..."/><div className="foundation-percent-input"><input type="number" min="0" max="100" value={item.progress} onChange={e=>changeFoundation(index,{progress:Number(e.target.value)})}/><span>%</span></div><button type="button" className="icon-button" onClick={()=>setFoundationUpdates(items=>items.filter((_,i)=>i!==index))}><Trash2 size={15}/></button>{f?<small>Hiện tại: {Number(f.progress).toFixed(0)}%</small>:null}</div>;})}</div><button className="button ghost" type="button" onClick={addFoundation}><Plus size={16}/> Chọn móng thi công hôm nay</button></>:<div className="empty-inline">Đội này chưa được giao mã móng.</div>}</section>

      <section className="form-section"><div className="section-heading"><span>05</span><div><h3>Công việc trong ngày</h3><p>Nội dung này sẽ đi nguyên vào báo cáo tổng hợp Ban điều hành.</p></div></div><div className="task-editor-stack">{tasks.map((item,index)=><div className="task-editor-card" key={index}><div className="task-editor-top"><select value={item.kind} onChange={e=>changeTask(index,{kind:e.target.value as "main"|"other"})}><option value="main">Công việc chính</option><option value="other">Công việc khác</option></select><input value={item.areaLabel} onChange={e=>changeTask(index,{areaLabel:e.target.value})} placeholder="Khu vực: Xưởng 1, Xưởng 3..."/><button type="button" className="icon-button" onClick={()=>setTasks(items=>items.filter((_,i)=>i!==index))}><Trash2 size={15}/></button></div><textarea rows={2} value={item.descriptionVi} onChange={e=>{const value=e.target.value;changeTask(index,{descriptionVi:value});scheduleTranslation(index,value);}} placeholder="Nội dung công việc bằng tiếng Việt"/><div className="translation-field-row"><Languages size={17}/><input value={item.descriptionZh} onChange={e=>changeTask(index,{descriptionZh:e.target.value,translationState:"done"})} placeholder="Tiếng Trung sẽ tự dịch theo nội dung tiếng Việt"/><button type="button" className="translation-retry" onClick={()=>translateTask(index,item.descriptionVi)} disabled={!item.descriptionVi.trim()||item.translationState==="loading"}>{item.translationState==="loading"?<Loader2 className="spin" size={15}/>:"Dịch lại"}</button></div><small className={`translation-status ${item.translationState||"idle"}`}>{item.translationState==="loading"?"Đang dịch...":item.translationState==="done"?"Đã dịch sang tiếng Trung. Có thể sửa lại nếu cần.":item.translationState==="error"?"Chưa dịch được tự động. Bạn có thể nhập thủ công hoặc bấm Dịch lại.":"Tiếng Trung sẽ được điền tự động sau khi nhập tiếng Việt."}</small></div>)}</div><button className="button ghost" type="button" onClick={()=>setTasks(items=>[...items,{kind:"main",areaLabel:"",descriptionVi:"",descriptionZh:"",translationState:"idle"}])}><Plus size={16}/> Thêm công việc</button></section>

      <section className="form-section"><div className="section-heading"><span>06</span><div><h3>Ảnh thi công đại diện</h3></div></div><label className="upload-box" onDragOver={e=>e.preventDefault()} onDrop={e=>{e.preventDefault();addFiles(e.dataTransfer.files);}}><UploadCloud size={30}/><strong>{files.length?"Chọn thêm ảnh công trường":"Chọn ảnh công trường"}</strong><span>JPG, PNG, WEBP, HEIC · tối đa 10 ảnh · có thể chọn nhiều lần hoặc kéo thả</span><input key={fileInputKey} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" multiple onChange={e=>addFiles(e.target.files)}/></label>{photoPreviews.length?<div className="photo-upload-preview-grid">{photoPreviews.map((item,index)=><div className="photo-upload-preview" key={`${fileKey(item.file)}-${index}`}><img src={item.url} alt={`Ảnh đã chọn ${index+1}`}/><button type="button" onClick={()=>removeFile(index)} aria-label={`Xóa ảnh ${index+1}`}><X size={16}/></button><span>{index+1}</span><small title={item.file.name}>{item.file.name}</small></div>)}</div>:null}{files.length?<div className="photo-selection-summary"><Camera size={16}/><strong>Đã chọn {files.length}/10 ảnh</strong><span>{files.length<10?`Có thể chọn thêm ${10-files.length} ảnh. Ảnh mới sẽ được cộng thêm, không thay ảnh cũ.`:"Đã đủ 10 ảnh."}</span></div>:null}<label className="field"><span>Chú thích ảnh</span><input value={photoCaption} onChange={e=>setPhotoCaption(e.target.value)} placeholder="VD: Lắp dựng cốt thép dầm móng Xưởng 1"/></label></section>
