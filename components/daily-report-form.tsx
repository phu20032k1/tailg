...
import { Camera, CheckCircle2, ClipboardPaste, Loader2, Plus, Trash2, UploadCloud, X } from "lucide-react";
  ...
  async function submit(event:FormEvent<HTMLFormElement>){
    ...
    try{
      const response=await fetch("/api/reports",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({
        reportDate, leaderId:user.role==="commander"?leaderId:undefined, rawMessage,
  ...
  return <>
    <form className="report-form daily-v3-form" onSubmit={submit}>
      <section className="form-section">
        <div className="section-heading"><span>01</span><div><h3>Tin nhắn báo cáo gốc</h3></div></div>
      ...
      <section className="form-section foundation-report-section">
        ...
        </>:<div className="empty-inline">Đội này chưa được giao mã móng. Chỉ huy trưởng có thể thêm trong mục Danh mục móng.</div>}
      </section>
      ...
      <section className="form-section">
        ...
        <label className="upload-box"><UploadCloud size={30}/><strong>Chọn ảnh công trường</strong><span>JPG, PNG, WEBP, HEIC · tối đa 10 ảnh</span><input key={fileInputKey} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" multiple onChange={e=>setFiles(Array.from(e.target.files||[]).filter(file=>file.type.startsWith("image/")).slice(0,10))}/></label>
