...
function createInitialLabor(): LaborEntry[] {
  return [
...
function createInitialEquipment(): EquipmentEntry[] {
  return [{ equipmentName: "Máy xúc", quantity: 0, unit: "máy" }];
}

function createInitialTasks(): TaskEntry[] {
  return [{ kind: "main", areaLabel: "", descriptionVi: "", descriptionZh: "" }];
}
...
export function DailyReportForm({ user, leaders }: { user: SessionUser; leaders: Leader[] }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const submittedWorkers = directWorkers;
    const submittedTechnical = technical;
    const submittedPhotos = files.length;
    ...
    try {
      ...
      if (reportId && files.length) {
        for (const file of files) {
          ...
          if (!upload.ok) throw new Error(`Báo cáo đã lưu nhưng ảnh tải lên chưa thành công: ${uploadResult.error || "Không rõ lỗi"}`);
        }
      }
      ...
      setNotice({
        ...
        text: `${submittedWorkers} công nhân · ${submittedTechnical} kỹ thuật${submittedPhotos ? ` · ${submittedPhotos} ảnh` : ""}`
      });
      router.refresh();
    } catch (error) {
      setNotice({
    ...
    <>
      <form className="report-form daily-v3-form" onSubmit={submit}>
        <section className="form-section">
          <div className="section-heading"><span>01</span><div><h3>Tin nhắn báo cáo gốc</h3><p>Dán nguyên tin nhắn nhóm dự án rồi bấm bóc tách.</p></div></div>
        ...
        <section className="form-section">
          ...
          <div className="summary-strip"><div><span>Công nhân trực tiếp</span><strong>{directWorkers}</strong></div><div><span>Kỹ thuật</span><strong>{technical}</strong></div><div><span>Tổng nhân lực nhập</span><strong>{labor.reduce((sum, item) => sum + Number(item.headcount || 0), 0)}</strong></div></div>
        ...
        <section className="form-section">
          <div className="section-heading"><span>05</span><div><h3>Ảnh thi công đại diện</h3></div></div>
          <label className="upload-box"><UploadCloud size={30} /><strong>Chọn ảnh công trường</strong><span>JPG, PNG, WEBP, HEIC · tối đa 10 ảnh · 10 MB/ảnh</span><input key={fileInputKey} type="file" accept="image/jpeg,image/png,image/webp,image/heic,image/heif" multiple onChange={(event) => setFiles(Array.from(event.target.files || []).filter((file) => file.type.startsWith("image/")).slice(0, 10))} /></label>
