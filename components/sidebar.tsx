...
const leaderNav: NavItem[] = [
  ...
  { href: "/commercial/materials", label: "Vật tư của đội", mobileLabel: "Vật tư", icon: PackageCheck },
  { href: "/commercial/subcontractors", label: "Theo dõi thanh toán thầu phụ", mobileLabel: "Thầu phụ", icon: ReceiptText },
...
function isActive(pathname: string, href: string) { if (href === "/") return pathname === "/"; if (href === "/commercial") return pathname === "/commercial"; return pathname === href || pathname.startsWith(`${href}/`); }
function navFor(user: SessionUser) { if (user.role === "commander") return commanderNav; if (user.role === "khkt") return khktNav; if (user.role === "director") return directorNav; if (user.role === "finance") return financeNav; return leaderNav; }

export function Sidebar({ user }: { user: SessionUser }) {
  const pathname = usePathname(); const [mobileOpen, setMobileOpen] = useState(false); const items = navFor(user); const primaryItems = items.slice(0, 4); const moreItems = items.slice(4); const moreActive = moreItems.some((item) => isActive(pathname, item.href));
  return <>
    <aside className="sidebar"><div className="sidebar-desktop-content"><div className="sidebar-brand"><span className="brand-logo small">T</span><div><strong>TAILG</strong><span>Điều hành công trường</span></div></div><div className="sidebar-project"><Building2 size={18}/><div><span>Dự án</span><strong>Nhà máy TAILG Việt Nam</strong></div></div></div><nav className="sidebar-nav">{items.map((item) => { const active = isActive(pathname,item.href); const Icon=item.icon; return <Link className={active?"nav-link active":"nav-link"} href={item.href} key={item.href}><Icon size={19} strokeWidth={1.9}/><span>{item.label}</span></Link>; })}</nav></aside>
    <nav className="mobile-tabbar" aria-label="Điều hướng chính trên điện thoại">{primaryItems.map((item)=>{const Icon=item.icon;const active=isActive(pathname,item.href);return <Link className={active?"mobile-tab-item active":"mobile-tab-item"} href={item.href} key={item.href} onClick={()=>setMobileOpen(false)}><Icon aria-hidden="true"/><span>{item.mobileLabel}</span></Link>;})}{moreItems.length?<button className={moreActive||mobileOpen?"mobile-tab-item active":"mobile-tab-item"} type="button" onClick={()=>setMobileOpen(true)} aria-label="Mở thêm chức năng" aria-expanded={mobileOpen}><MoreHorizontal aria-hidden="true"/><span>Thêm</span></button>:null}</nav>
